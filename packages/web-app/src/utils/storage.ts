...
export class SettingsStorage extends Dexie {
  settings: Dexie.Table<SettingString, string>; // string = type of the primkey
  settingFlags: Dexie.Table<SettingFlag, string>;
  ...
  constructor() {
    ...
    this.settingFlags = this.table("settingFlags");
  }
  ...
  setFlag(key: string, value: boolean): Promise<any> {
    return this.settingFlags.update(key, { value })
      .catch((err) => {
        console.error("Incognito mode might be on", err);
        throw err;
      });
  ...
  getFlag(key: string): Promise<boolean> {
    return this.settingFlags.get(key)
  ...
  allFlags(): Promise<SettingFlag[]> {
    return this.settingFlags.toArray()
      .catch((err) => {
        console.error("Incognito mode might be on", err);
        throw err;
      });
...
const mySettings = new SettingsStorage()
await mySettings.set("name", "John")
await mySettings.set("lastName", "Smith")
await mySettings.get("name")
await mySettings.get("lastName")
...
await mySettings.getFlag("essential-cookies")
await mySettings.getFlag("optional-cookies")
await mySettings.getFlag("testing") // false

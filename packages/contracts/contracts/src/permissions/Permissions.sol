...
contract Permissions is UpgradableComponent {
    bytes32 public constant PERMISSIONS_SET_ROLE =
        keccak256('PERMISSIONS_SET_ROLE');
    ...
    function setRole(string calldata role, Permission calldata permission)
        external
        authP(PERMISSIONS_SET_ROLE)
    {
    ...
    function checkPermission(string calldata role)
        external
        view
        returns (bool valid)
    {

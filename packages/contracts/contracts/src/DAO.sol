...
contract DAO is IDAO, Initializable, UUPSUpgradeable, ACL {
    ...
    event NewProposal(
        GovernancePrimitive.Proposal indexed proposal,
        Processes.Process indexed process,
    ...
    function _authorizeUpgrade(
        ...
        require(
            willPerform(address(this), msg.sender, UPGRADE_ROLE, msg.data),
            'auth:check'
        );
    }
    ...
    modifier authP(bytes32 role) {
        require(
            willPerform(address(this), msg.sender, role, msg.data),
            'auth: check'
        );
        _;
    }
    ...
    function checkPermission(string calldata _role)
        external
        view
        override
        returns (bool)
    ...
    function start(GovernancePrimitive.Proposal calldata proposal)
        external
        returns (Processes.Process memory process, uint256 executionId)
    {
    ...
    function execute(
        uint256 executionID,
        GovernancePrimitive governancePrimitive
    ) external {
    ...
    function addRole(
        ...
        Permissions.Permission calldata permission
    ) external authP(DAO_CONFIG_ROLE) {

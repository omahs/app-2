...
contract SimpleVoting is VotingGovernancePrimitive, UpgradableComponent {
    ...
    function initialize(
        ...
        uint64[3] calldata _voteSettings
    ) public initializer {
        token = _token;
    ...
    function _start(Execution memory execution) internal override {
        (string memory description, bool executeIfDecided, bool castVote) = abi
            .decode(
                execution.proposal.additionalArguments,
                (string, bool, bool)
            );
    ...
    function _vote(uint256 _voteId, bytes calldata data) internal override {
        (bool _supports, bool _executesIfDecided) = abi.decode(
            data,
            (bool, bool)
        );
    ...
    */
     ...
     */
    function _isVoteOpen(Vote storage vote_, uint256 voteId)
        internal
        view
        returns (bool)
    {
        return
            getTimestamp64() < vote_.startDate + voteTime &&
            _isVoteExecuted(voteId);
    }
